...
class InvoicesController extends Controller
{
    ...
    //     $account = DB::table('accounts')->where('id', $request->acc_id)->first();
    //     if (!$account) {
    ...
    //         'prev_balance'     => $account->prev_balance,
    //         'current_balance'  => $account->prev_balance - $request->total_return,
    ...
    //             ->decrement('prev_balance', $request->total_return);
    //     }
    //     $selectedCount = 0;
    ...
    //             $total = $qty * $price;
    //             $invoiceItem = DB::table('invoice_items')->where('id', $itemId)->first();
    //             $invoice     = DB::table('invoices')->where('id', $invoiceItem->invoice_id)->first();
    //             DB::table('return_items')->insert([
    //                 'item_name'    => $invoiceItem->name,
    //                 'item_id'      => $invoiceItem->stock_id,
    //                 'invoice_to'   => $account->customer_name,
    //                 'item_price'   => $invoiceItem->final_price,
    //                 'qty'          => $qty,
    //                 'total'        => $total,
    //                 'created_at'   => now(),
    //                 'updated_at'   => now(),
    ...
    //                 DB::table('invoice_items')->where('id', $itemId)->update([
    //                     'status' => 'Partial Returned',
    //                     'partial_qty' => DB::raw('COALESCE(partial_qty, 0) + ' . $qty),
    //                 ]);
    //                 $returnableQty = $invoiceItem->qty - (($invoiceItem->partial_qty ?? 0) + $qty);
    //                 if ($returnableQty <= 0) {
    //                     DB::table('invoice_items')->where('id', $itemId)->update([
    //                         'status' => 'Returned',
    //                     ]);
    ...
    //         }
    //     }
    //     DB::table('return_invoices')
    //         ->where('id', $returnInvoiceId)
    //         ->update(['total_items' => $selectedCount]);

    //     return redirect()->route('invoice.return.view', ['id' => $returnInvoiceId])
    //         ->with('success', 'Return invoice created with items and account balance updated.');
    // }
    ...
    public function savereturn(Request $request)
    {
        ...
        if ($request->has('items')) {
            foreach ($request->items as $itemId => $itemData) {
                ...
                $invoiceItem = DB::table('invoice_items')->where('id', $itemId)->first();
                $invoice     = DB::table('invoices')->where('id', $invoiceItem->invoice_id)->first();
                $stock       = DB::table('stocks')->where('id', $invoiceItem->stock_id)->first();

                // Calculate lost profit: (selling price - purchase price) * returned qty
                $lostProfit = ($price - $stock->purchase_price) * $qty;
                ...
                // Update stock (increase qty back)
                DB::table('stocks')
                    ->where('id', $invoiceItem->stock_id)
                    ->increment('qty', $qty);
                ...
                // Deduct lost profit from invoice total profit
                DB::table('invoices')
                    ->where('id', $invoice->id)
                    ->decrement('profit', $lostProfit);
            }
        }
        ...
        return redirect()->route('invoice.return.view', ['id' => $returnInvoiceId])
            ->with('success', 'Return invoice created, items processed, profit adjusted, and account balance updated.');
    }
        ...
        if (Gate::allows('is_admin')) {
            ...
            if ($request->acc_id) {
                $items = DB::table('invoice_items')
                    ...
                    ->select('invoice_items.name as stock_name', 'invoice_items.id as id', 'invoice_items.status as status', 'invoice_items.qty as stock_qty', 'invoice_items.final_price as price', 'invoice_items.total as total', 'invoice_items.partial_qty as partial_qty')
                    ->where('invoices.account_id', $request->acc_id)
            ...
            return view('invoices.return.make', compact('accounts', 'items'));
        } else {
            return abort(401);
        }
    }
