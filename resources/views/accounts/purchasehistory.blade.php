...
@php
    $title = 'Purchase History | ' . $account->customer_name;
@endphp
    ...
    <div class="container-fluid px-3">
            ...
            <div class="row">
                ...
                <div class="col-lg-10 col-md-9 col-6 col-sm-8">
                    <h3 class="mt-1 d-none d-md-block d-lg-block" style="font-family:cursive">
                        {{ 'Purchase History | ' . $account->customer_name }}</h3>

                    <h5 class="mt-1 d-block d-md-none d-lg-none" style="font-family:cursive">
                        {{ 'Purchase History | ' . $account->customer_name }}</h5>
                </div>
            </div>
        </div>
        ...
        <div class="card mb-2 p-2 mt-2">
            <form action="" method="GET">
                <div class="row">
                    <div class="col-lg-4 col-md-2 col-sm-6 col-6 mt-1 mb-1">
                        <input type="text" class="form-control" placeholder="Name" value="{{ request()->name }}"
                            name="name">
                    </div>
                    ...
                    <div class="col-lg-3 col-md-4 col-sm-6 col-6 mt-1 mb-1">
                        <div class="btn-group w-100">
                            <a href="{{ url('admin/accounts/view/' . request()->id . '/purchase-history') }}" title="Clear"
        ...
        @if ($items->count() > 0 && (request()->date || request()->name || request()->month))
            <div class="alert bg-primary text-white mt-3">
                <strong>{{ $items->count() }}
                    {{ $items->count() > 0 && $items->count() < 2 ? 'Result' : 'Results' }}
                    Found</strong>
            </div>
        @elseif ($items->count() < 1 && (request()->date || request()->name || request()->month))
                    ...
                    <table class="table">
                        <thead>
                            <tr>
                                ...
                                <th style="font-size:14px;width:12%" class="text-dark text-center fw-bold">Invoice No</th>
                                <th style="font-size:14px;width:12%" class="text-dark fw-bold text-center">Date</th>
                                <th style="font-size:14px;width:10%" class="text-dark fw-bold text-center">Action</th>
                            </tr>
                        </thead>
                        <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-dark text-center">
                                        ...
                                        <a href="{{ route('invoice.view', ['id' => $item->invoice_no]) }}" target="_Blank"
                                            class="btn btn-xs btn-primary"> {{ $item->invoice_no }} | View</a>
                                    </td>
                                    <td class="text-dark text-center">
                                    ...
                                    <td class="text-dark text-center">
                                        @if ($item->item_stock_id != null && $item->status != 'Returned')
                                            <a href="#" data-bs-toggle="modal" data-bs-target="#returnModal"
                                                ...
                                                data-item-qty="{{ $item->qty }}" data-item-status="{{ $item->status }}"
                                                data-item-partial-qty="{{ $item->partial_qty ?? 0 }}"
                                                data-item-price="{{ $item->price }}" class="btn btn-danger btn-xs">Return
                ...
                <div class="table-responsive">
                    <table class="table">
                        <thead>
                            ...
                            <th style="font-size:14px;" class="text-dark fw-bold text-center">Name
                            </th>
                            <th style="font-size:14px" class="text-dark fw-bold">Price</th>
                            <th style="font-size:14px;" class="text-dark fw-bold text-center">Qty
                            </th>

                            <th style="font-size:14px;" class="text-dark fw-bold text-center">Total
                            </th>
                            <th style="font-size:14px;" class="text-dark fw-bold text-center">Invoice No
                            </th>
                            <th style="font-size:14px" class="text-dark fw-bold text-center">Date</th>
